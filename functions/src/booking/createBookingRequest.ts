...
export const createBookingRequest = functions.https.onCall(async (data, context) => {
  ...
  const { 
    clientName, contactPhone, eventType, location, 
    startDateTime, endDateTime, equipmentRequested 
  } = data;

  const db = admin.firestore();
  
  // Extract just the IDs for the array-contains-any query
  const equipmentIds = equipmentRequested.map((item: any) => item.equipmentId);
