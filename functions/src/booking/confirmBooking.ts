...
export const confirmBooking = functions.https.onCall(async (data, context) => {
  ...
  return db.runTransaction(async (tx) => {
    ...
    const overlapping = overlappingSnapshot.docs
      ...
      .filter((b: any) => b.endDateTime.toMillis() > booking.startDateTime.toMillis() && b.id !== bookingId);

    const committedQuantity: Record<string, number> = {};
    booking.equipmentIds.forEach((id: string) => committedQuantity[id] = 0);
    
    for (const b of overlapping) {
      for (const item of (b as any).equipmentRequested) {
